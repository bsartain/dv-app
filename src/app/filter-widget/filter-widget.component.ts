...
export class FilterWidgetComponent implements OnInit {
  ...
  //private _objectType: ObjectType = null;

  //fid2allCheckH: { [id_Query_Filter_ID: number] : boolean } = {};

  constructor(private _qpSvc: QueryPageService,
    private _element: ElementRef,
    private zone:NgZone // Used to fix strange problem with popover
    ) { }

  ngOnInit() {
    //this._objectType = this._qpSvc.ftrSvc.objectTypeC.find(this.objectTypeID);
    // console.log(this._qpSvc.ftrSvc.objectTypeC.find(this.objectTypeID).filterC.array().length)
    // this._qpSvc.ftrSvc.objectTypeC.find(this.objectTypeID).filterC.array().forEach(
    //   ftr => {
    //     console.log("scott: " + ftr.txt_Header_Name)
    //   }
    // )
    //console.log('brett3333333333: ' + JSON.stringify(this._qpSvc.ftrSvc.objectTypeC.find(this.objectTypeID).filterC.array()))
  }
  ...
  onChangeCheckbox1(ftr: Filter, fv:Query__Filter_Value) {
    ...
    console.log('ftr ' + JSON.stringify(curr));
    console.log('ftr 2 ' + JSON.stringify(ftr));
  }
